...
import numpy as np
from .motion_planning_4 import Motion_planning
...
20251128情况：上述轴孔装配收敛性判断、插孔路径规划和判断轴孔装配是否成功代码的有效性已经进行了实验验证，除了在判断装配是否成功的阈值还不是很确定；下一步需要改进主动探索的方向选择。
20251213情况：在更改了竖直力过小判据后，需要重新标定粒子范围的边缘坐标；在落入孔内后，如果插入没有成功，后续的探索代码需要进一步编写（完成）。
20251214情况：对于孔外形测量的误差过大（长度误差5mm）导致粒子滤波失败，关键是没有在每一步后进行重采样；再者，测量误差不可避免，能否通过优化的方式消除也是需要考虑的。
20251218情况：每一步的重采样数量需要精细考虑（已解决）；主动探索的信息计算需要进一步改进（已解决）；对于粒子收敛后，如何完成最终的装配也是需要考虑的(已解决)
20251227情况：需加入最终插入过程的粒子位置跟随。
20260103总结：对于轴孔装配实验的定位和完成来说，需要正确地检测接触事件（灵活性还需进一步提高），对于环境建模存在误差的情况下，需要在粒子滤波时对可容忍的阈值范围有一个更精确的判断，一是粒子的权重
需要保持连续更新而不是每次迭代后重置均值，这样能够在多次的探索中对误差进行累积(已完成)；二是需要最大限度减缓不确定性测量的误差，最紧迫的是对夹持物体的运动趋势进行建模并引入粒子的位置分布中(已完成)，这个误差虽小
但是有时能够影响迭代结果；三是对于客观存在的不确定性进行推理和主动衰减，这样建图的对象就不只是相对位置了，还包括原图的修正。
20260111总结：对于信息增益的计算不够准确，有一些粒子在运动后会进入轮廓内部但是接触比例太少从而也不会被过滤掉的也会被在计算信息增益时被当作过滤掉的粒子
20260115总结：对于信息增益的计算来说，有时出现运动范围过于狭小的情况，这不利于探索效率，且会造成误差的累积，思考如何“从粗到细”快速减少探索空间(已解决)。可以再加一个底线的方法，当确实陷入局部凹陷后
"""
...
class MinimumJerkPosePlanner(Node):
        ...
        if self.initial is not None and self.target is not None:  ##满足轨迹规划的基本条件
            ...
            if self.N == 0:  ## 初始时机械臂向下运动直到接触孔平面
                ...
                if self.dz_flag == 0:  ##粒子分布收敛，准备运动到孔附近上方
                    ...
                    ## 计算粒子的当前位置与目标位置之间的差值
                    ## 运动
                    target_pose = self.hole_pose + np.array([0.0, -3.0])  ##粒子先运动到孔位置一侧的上方
            ...
            elif self.N == 5: ##优化策略，直接运动到插孔位置
                if self.dz_flag == 0:  ##准备运动到孔附近上方
                    # 根据centers候选位置选择
                    pose_ref, particle_left, weight_left = self.M.gain_information_optimize(Object=self.Movable_objects,
                    ...
                    ## 运动
                    target_pose = self.hole_pose + np.array([0.0, -3.0])  ##粒子先运动到孔位置一侧的上方
                    delta = -(pose_ref - target_pose) / 1000  ##这里的单位为m
                    self.delta_N5 = delta
                    self.particles = particle_left  ##更新粒子的位置(单位为mm)
                    self.weights = weight_left
                    print(f'len(particles)={self.particles.shape}')
                    self.get_logger().info(f"The current pin pose: {pose_ref}.")
                    self.get_logger().info('Move the particles to ahead of hole!')
                    Points_recover.append(start + np.array([0.5 * delta[0], 0.5 * delta[1], 0.06]))  ##适当向上运动
                    Points_recover.append(start + np.array([delta[0], delta[1], 0.06]))
                    self.v_pred = np.array([0, 0])  ##末端只在竖直方向上运动，横向速度均为0
                    self.N_ite = 0
                if self.dz_flag == 1:  ##已运动到目标位置上方，向下运动接触孔平面
                    Points_recover.append(start + np.array([0.0, 0.0, -0.04]))  # 这里的单位是m
                    Points_recover.append(start + np.array([0.0, 0.0, -0.064]))
                    self.get_logger().info('-------Contact!!!-----------')
                    self.v_pred = np.array([0, 0])  ##末端只在竖直方向上运动，横向速度均为0
                if self.dz_flag == 2:  ## 接触孔平面后，，向孔方向运动，同时检测是否符合约束条件(约束条件就是Z方向无力，横向有约束)
                    self.v_pred = np.array([0,1])
                    self.get_logger().info(f"The chosen velocity is:{self.v_pred}")
                    self.N = 1  ##此时就进入到N1环节，开始正常进行粒子滤波与探索
                    while True:
                        user_in = input()
                        if user_in == 'q':
                            break
                    ...
                    self.get_logger().info('-------To Find Constrain!!!-----------')
                    self.N_ite += 1
                    self.v_move = 0.0015 ##此时的移动速度降为原速度的三分之一

                self.get_logger().info("The current state is N=5.")
                self.get_logger().info(f"The current dz_flag={self.dz_flag}")
    ...
    def publish_at_time(self):
        ...
        if all(v is not None for v in (self.Fr_r, self.Fr_l, self.Mr_r, self.Mr_l)):
            ...
            ##计算moving trend
            l_curr_l = (self.world_to_gripper_translation * self.gripper_to_sensor_translation * self.sensor_to_tool_translation_left * geo.V4(
                               0, 0, 0, 1))[0: 3]
            l_curr_r = (self.world_to_gripper_translation * self.gripper_to_sensor_translation * self.sensor_to_tool_translation_right * geo.V4(
                               0, 0, 0, 1))[0: 3]
            l_cuur = (l_curr_l + l_curr_r) / 2
            l_cuur = np.array(l_cuur)
            Pose_tool_curr = l_cuur / 1000.0 ## 单位为mm，转化为m
            self.Pose_tool_all = np.append(self.Pose_tool_all, Pose_tool_curr).reshape(-1, 3)
            ...
            if self.force_control_flag:  ##接触力调整与推理阶段，保持一个恒定的接触力
                ...
                if self.N == 5:
                    ...
                    ##保存相关数据进行分析
                    np.savez(f'contact_slam_N5data_2511101625_{str(self.num)}.npz', particles=self.particles,
                                     weights=self.weights, Pose_list=self.Pose,
                                     Pose_tool_list=np.array(self.Pose_tool, dtype=np.float64),
                    ...
                    self.get_logger().info(f'The N5 data has been saved!')
                ...
                if self.N == 4:
                    ...
                    np.savez(f'contact_slam_N4data_2511101625_{str(self.num)}.npz', particles=self.particles,
                                     weights=self.weights, Pose_list=self.Pose_pre, Pose_tool_list=np.array(self.Pose_tool_pre, dtype=np.float64),
                                     particles_filter=self.particles_filter,
                ...
                if self.N == 3: ## 还未收敛时落入孔内判断是否与孔对齐
                    ...
                    if d_height > 0.02: ##具体数值可再修改
                        ...
                        self.Pose_tool = np.array(self.Pose_tool, dtype=np.float64)
                        self.Pose_tool_pre = np.array(self.Pose_tool_pre, dtype=np.float64)
                        if np.max(abs(self.Pose_tool - self.Pose) * 1000) <= 10:
                            dis = (self.Pose_tool_pre[-1] - self.Pose_tool_pre[0]) * 1000
                            pose_list = self.Pose_tool_pre * 1000
                            self.get_logger().info(f'The moving trend estimation is Feasible!')
                            self.get_logger().info(f'dis={dis}')
                        else:
                            dis = (self.Pose_pre[-1] - self.Pose_pre[0]) * 1000
                            pose_list = self.Pose_pre * 1000
                            self.get_logger().info(f'The moving trend estimation is Unfeasible!')
                        self.particles, self.weights, particles_remain = self.M.update_particles(self.particles, self.weights,
                                                                               particle_range, self.Z_obs_pre,
                                                                               shift_3=dis)
                        # 更新权重
                        ...
                        if std[0] < 0.9 and std[1] < 0.9:
                            ...
                            self.particles, self.weights = self.M.resample_particles(self.particles_filter, self.weights_filter, particles_remain)
                    ...
                    np.savez(f'contact_slam_N3data_2511101625_{str(self.num)}.npz', particles=self.particles,
                             weights=self.weights, Pose_list=self.Pose_pre, Pose_tool_list=np.array(self.Pose_tool_pre, dtype=np.float64), particles_filter=self.particles_filter,
                             Z_list=self.Z_obs_pre, Time=self.Time, v_pred=self.v_pred_pre, scene=self.scene,
                             xs=self.xs, ys=self.ys)
                ...
                if self.N == 2:
                    ...
                    self.Pose_tool = np.array(self.Pose_tool, dtype=np.float64)
                    if np.max(abs(self.Pose_tool - self.Pose) * 1000) <= 10:
                        dis = (self.Pose_tool[-1] - self.Pose_tool[0]) * 1000
                        pose_list = self.Pose_tool * 1000
                        self.get_logger().info(f'The moving trend estimation is Feasible!')
                        self.get_logger().info(f'dis={dis}')
                    else:
                        dis = (self.Pose[-1] - self.Pose[0]) * 1000
                        pose_list = self.Pose * 1000
                        self.get_logger().info(f'The moving trend estimation is Unfeasible!')
                    ...
                    if std[0] < 0.9 and std[1] < 0.9:
                        ...
                        self.particles, self.weights = self.M.resample_particles(self.particles_filter, self.weights_filter,
                                                                                 particles_remain)
                        current_pose = np.array(
                            [self.current.position.x, self.current.position.y])
                        delta_init = -(current_pose - self.Pose[0])  ##这里单位为m
                        particles_init = self.particles + delta_init * 1000  ##更新粒子的位置，将粒子的分布也返回上一次出发点（单位转化成mm)
                        ## 将粒子限制在狭义运动范围内
                        particle_range = [np.array(
                            [[self.M.x_min_socket, self.M.y_min_socket], [self.M.x_max_socket, self.M.y_min_socket],
                             [self.M.x_max_socket, self.M.y_max_socket], [self.M.x_min_socket, self.M.y_max_socket]])]
                        inside_mask_total = np.zeros(len(particles_init), dtype=bool)
                        for poly in particle_range:
                            path = Path(poly)
                            inside_mask_total |= path.contains_points(particles_init)
                        particles_init = particles_init[inside_mask_total]
                        self.weights = self.weights[inside_mask_total]
                    ...
                    np.savez(f'contact_slam_N2data_2511101625_{str(self.num)}.npz', particles=self.particles,
                             weights=self.weights, Pose_list=self.Pose, Pose_tool_list=np.array(self.Pose_tool, dtype=np.float64), particles_filter=self.particles_filter,
                             Z_list=self.Z_obs, Time=self.Time, v_pred=self.v_pred, scene=self.scene,
                             xs=self.xs, ys=self.ys)
                    ...
                    if self.mu * f_ref * 0.9 < np.linalg.norm(
                        ...
                        if self.N == 0:
                            ...
                            np.savez(f'contact_slam_N0data_2511101625_{str(self.num)}.npz', particles=self.particles,
                                     weights=self.weights, Pose_list=self.Pose, Pose_tool_list=np.array(self.Pose_tool, dtype=np.float64),
                                     Z_list=self.Z_obs, Time=self.Time, v_pred=self.v_pred, scene=self.scene,
                                     xs=self.xs, ys=self.ys)
                            ...
                            if std[0] < 0.9 and std[1] < 0.9:
                                ...
                                self.particles, self.weights = self.M.resample_particles(self.particles_filter, self.weights_filter,
                                                                                         particles_remain)
                                ## 将粒子限制在狭义运动范围内
                                particle_range = [np.array([[self.M.x_min_socket, self.M.y_min_socket], [self.M.x_max_socket, self.M.y_min_socket],
                                                            [self.M.x_max_socket, self.M.y_max_socket], [self.M.x_min_socket, self.M.y_max_socket]])]
                                inside_mask_total = np.zeros(len(self.particles), dtype=bool)
                                for poly in particle_range:
                                    path = Path(poly)
                                    inside_mask_total |= path.contains_points(self.particles)
                                self.particles = self.particles[inside_mask_total]
                                self.weights = self.weights[inside_mask_total]
                                self.weights /= np.sum(self.weights)
                                ## 综合探索与优化，判断最优的运动方向
                                peaks = self.M.find_local_maximum(self.particles, self.weights, radius=5)
                ...
                if self.X is not None:
                    if self.n_now < self.X.shape[0]:  ##轨迹追踪还没有进行完
                        ...
                        ##合力在运动方向上的分量
                        f_projec = (f_vec @ self.v_vec) / (self.v_vec @ self.v_vec) * self.v_vec
                        if self.N == 0 or self.N == 4 or self.N == 5:
                            f_ref = 2.0
                        elif self.N == 3:
                            f_ref = 3.0
                        else:
                        ...
                        if np.linalg.norm(f_projec) > self.mu * f_ref and (f_vec @ self.v_vec) < 0 and np.linalg.norm(
                            ...
                            self.Pose_tool = np.append(self.Pose_tool, Pose_tool_curr[:2]).reshape(-1, 2)
                            ...
                            if self.n_now < self.X.shape[0]:  ##轨迹还未执行完，继续执行轨迹
                                if self.terminate_flag:  ##进入接触状态核查阶段（核查是否落入了孔区域，是否超出了运动区域）
                                    ...
                                    if f_z > 1.0:  ##还存在接触
                                        ...
                                        if abs(self.v_pred[1]) == 1 and (self.v_pred[1] * f_y) < -1.0:
                                            self.dz_flag = 1
                                        if abs(self.v_pred[0]) == 1 and (self.v_pred[0] * f_x) < -0.9:
                                            self.dz_flag = 1
                                        if self.dz_flag == 1: # 横向受到了约束，说明落入了孔约束，下一步尝试向下插入（需进一步完善，比如x方向落入孔区域怎么办）
                                            ...
                                            self.Pose_tool_pre = self.Pose_tool
                                ...
                                if self.i % 100 == 0:
                                    ...
                                    self.get_logger().info(f'Pose_tool={self.Pose_tool[-1]}')
                            else:
                                self.cmd.position.x += delta_x / self.freq
                                self.cmd.position.y += delta_y / self.freq
                                self.cmd.position.z += delta_z / self.freq
                        ...
                        if self.N == 4 and self.dz_flag == 0:  ## N4模式下，此时移动到了目标位置的上方，但还没有插入
                            ...
                            self.X, self.Y, self.Z = None, None, None
                            self.force_inside_flag = False
                        elif self.N == 5 and self.dz_flag == 0:  ## N5模式下，此时移动到了目标位置的上方，但还没有插入
                            self.get_logger().info('insert but haven\'t contact with surface')
                            ## 重置以接收新目标
                            self.dz_flag = 1 ##下一步准备向下运动运动
                            self.initial = None
                            ...
                            self.X, self.Y, self.Z = None, None, None
                            self.force_inside_flag = False
                        elif self.N == 5 and self.dz_flag == 1: ## N5模式下，此时说明向下运动没有接触到孔平面
                            self.get_logger().info('Haven\'t find the hole plane')
                            self.N = 2 ##返回上一次的出发点，这是必要的，因为夹爪可能超出了运动范围
                            self.dz_flag = 0
                            ## 重置以接收新目标
                            self.initial = None
                            self.total_time = 0.0
                            ...
                            ## 重置以接收新目标
                            self.get_logger().info('N2 Trajectory complete and has returnd to the initial pose.')
                            self.N = 0  ##重新向下运动进行探索
                            self.initial = None
                            self.total_time = 0.0
                            self.n_now = 0
...
def main(args=None):
    ...
    try:
        ...
        np.savez(r'Force_data_2511101625.npz', Force_left=to_numpy_float(node.Force_left), Force_right=to_numpy_float(node.Force_right),
                 Matrix_left=to_numpy_float(node.Matrix_left), Matrix_right=to_numpy_float(node.Matrix_right), Force_dis_left=to_numpy_float(node.Force_dis_left),
                 Force_dis_right=to_numpy_float(node.Force_dis_right),
                 F_x=node.F_x, F_y=node.F_y, F_z=node.F_z, R_x=node.R_x, R_y=node.R_y, R_z=node.R_z,
                 Pose=node.Pose_all, Pose_tool=to_numpy_float(node.Pose_tool_all), Time=node.Time)
        print('save Data!')
        node.destroy_node()
        rclpy.shutdown()
