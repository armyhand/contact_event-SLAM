...
class Motion_planning:
    def __init__(self, d, padding):
        ...
        self.r = 7.0  ##重采样半径范围
        self.belief_resolution = max(1.0, self.d)
        self.distance_resolution = max(1.0, 2 * self.d)
        ## Lazy RTDP-Bel 参数。先用 belief 子样本估计 Q，再按需展开候选动作。
        self.q_hat_sample_size = 12
        self.lazy_q_values = {}
        self.lazy_evaluated_actions = {}
        self.belief_values = {}
    ...
    def belief_from_particles(self, particles, weights):
        ...
        cells = np.rint(particles / resolution).astype(int)
        ...
        hypotheses = np.zeros((len(unique_cells), 2), dtype=float)
        for i in range(len(unique_cells)):
            ids = inverse == i
            hypotheses[i] = np.average(particles[ids], axis=0, weights=weights[ids])
        keep = cell_weights >= max(1e-6, 0.25 / len(unique_cells))
        hypotheses, cell_weights = hypotheses[keep], cell_weights[keep]
        cell_weights /= max(np.sum(cell_weights), 1e-12)
        key_cells = np.rint(hypotheses / self.belief_resolution).astype(int)
        return tuple(map(tuple, key_cells.tolist())), hypotheses, cell_weights

    def _contact_signature(self, hypothesis, action, objects):
        """预测位姿假设沿 blind-push 动作首次产生的观测与距离类别。"""
    ...
    def _domain_heuristic(number_of_hypotheses):
        ## 二值接触观测每步最多二分 belief，log2(|H|) 是 admissible 下界。
        return 0.0 if number_of_hypotheses <= 1 else math.log2(number_of_hypotheses)

    def _lazy_partition(self, ids, hypotheses, action, objects):
        branches = {}
        for i in ids:
            observation = self._contact_signature(hypotheses[i], action, objects)
            branches.setdefault(observation, []).append(i)
        return branches

    def _q_hat_init(self, ids, hypotheses, probabilities, action, objects):
        """用固定子样本快速估计 Q，仅用于未展开动作的初始排序。"""
        sample_count = min(self.q_hat_sample_size, len(ids))
        positions = np.linspace(0, len(ids) - 1, sample_count, dtype=int)
        sample_ids = tuple(ids[i] for i in positions)
        branches = self._lazy_partition(sample_ids, hypotheses, action, objects)
        sample_weights = probabilities[list(sample_ids)]
        total = max(np.sum(sample_weights), 1e-12)
        estimate = 1.0
        for branch in branches.values():
            probability = np.sum(probabilities[branch]) / total
            estimated_size = max(1, int(round(len(ids) * probability)))
            estimate += probability * self._domain_heuristic(estimated_size)
        return estimate

    def _evaluate_lazy_action(
        self, belief_key, hypotheses, probabilities, action, objects
    ):
        """完整生成一个动作的后继 belief，并执行真实 Bellman backup。"""
        ids = tuple(range(len(belief_key)))
        branches = self._lazy_partition(ids, hypotheses, action, objects)
        total = max(np.sum(probabilities), 1e-12)
        value = 1.0
        for branch in branches.values():
            successor_key = tuple(belief_key[i] for i in branch)
            successor_value = self.belief_values.get(
                successor_key, self._domain_heuristic(len(successor_key))
            )
            value += np.sum(probabilities[branch]) / total * successor_value
        self.transition_evaluations += 1
        return value

    def lazy_rtdp_bel_action(self, particles, weights, objects):
        """Algorithm 6：惰性评估候选动作，直到当前最优动作稳定。"""
        ...
        action_keys = [tuple(action.tolist()) for action in self.actions]
        q_values = self.lazy_q_values.setdefault(belief_key, {})
        evaluated = self.lazy_evaluated_actions.setdefault(belief_key, set())
        for action, action_key in zip(self.actions, action_keys):
            if action_key not in q_values:
                q_values[action_key] = self._q_hat_init(
                    tuple(range(len(belief_key))),
                    hypotheses,
                    probabilities,
                    action,
                    objects,
        ...
        for action, action_key in zip(self.actions, action_keys):
            if action_key in evaluated:
                q_values[action_key] = self._evaluate_lazy_action(
                    belief_key, hypotheses, probabilities, action, objects
                )

        evaluated_now = 0
        while True:
            best_key = min(
                action_keys, key=lambda key: (q_values[key], action_keys.index(key))
            )
            if best_key in evaluated:
                break
            action = np.asarray(best_key, dtype=int)
            q_values[best_key] = self._evaluate_lazy_action(
                belief_key, hypotheses, probabilities, action, objects
            )
            evaluated.add(best_key)
            evaluated_now += 1

        self.belief_values[belief_key] = q_values[best_key]
        action = np.asarray(best_key, dtype=int)
        print(
            f"Lazy RTDP-Bel: hypotheses={len(belief_key)}, action={action}, "
            f"evaluated={evaluated_now}/{len(self.actions)}, Q={q_values[best_key]:.3f}"
        )
        return action
...
def scene_plot(
    ...
    if folder is not None:
        plt.savefig(
            os.path.join(folder, f"contact_slam_data_2511101625_{step}.png"),
            dpi=300,
            bbox_inches="tight",
        )
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    for p in range(19, 25):
        # 每次迭代的新目录
        folder = f"lazy_search_push/ablation_group2_{p}"  # 或用时间戳
        ...
        # v_pred = M_planning.lazy_rtdp_bel_action(
        #     particles, weights, Movable_objects
        # )
        v_pred = np.array([0, 1])
        while (
