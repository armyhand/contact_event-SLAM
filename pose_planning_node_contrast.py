...
class MinimumJerkPosePlanner(Node):
    ...
    def As_planner(self):
        if self.initial is not None and self.target is not None:  ##满足轨迹规划的基本条件
            ...
            if self.N == 0:  ## 初始时机械臂向下运动直到接触孔平面
            ...
            elif self.N == 1:  ##生成螺旋搜孔的轨迹
                self.dz_flag = 0
                self.get_logger().info("Start spiral hole search planning...")
                max_radius = 0.04 ## 最大半径
                pitch = 0.002 ## 每圈的增量
                d_theta = np.pi / 30 ## 角度的步进
                ...
                while r < max_radius:
                    ...
                    dx = r * np.cos(theta)
                    dy = r * np.sin(theta)
                    Point = Point + np.array([dx, dy, 0.0])

                    # 加入到轨迹规划中
                    Points_recover.append(Point)
                    theta += d_theta # 更新角度
                self.get_logger().info("Finish spiral hole search planning...")
                # ===== 新增: 等待键盘输入 'q' 才继续 =====
                self.get_logger().info(f'Points_recover = {Points_recover}')
                np.savez(f'spiral_path.npz', path=Points_recover)
    ...
    def publish_at_time(self):
        ...
        if all(v is not None for v in (self.Fr_r, self.Fr_l, self.Mr_r, self.Mr_l)):
            ...
            delta_x = np.sign(f_x - 0.05) * min(0.009, 0.009 * abs(f_x - 0.05)) if abs(f_x) > 0.1 else 0.0
            delta_y = np.sign(f_y - 0.1) * min(0.009, 0.009 * abs(f_y - 0.1)) if abs(f_y) > 0.2 else 0.0
            ...
            if self.force_control_flag:  ##接触力调整与推理阶段，保持一个恒定的接触力
                ...
                if self.N == 4:
                    ...
                    ##保存相关数据进行分析
                    np.savez(f'contact_slam_N4data_2511101625_{str(self.num)}.npz', Pose_list=self.Pose_pre,
                                    Time=self.Time, v_pred=self.v_pred_pre, scene=self.scene,
                ...
                if self.N == 3: ## 还未收敛时落入孔内判断是否与孔对齐
                    ...
                    ##保存相关数据进行分析
                    np.savez(f'contact_slam_N3data_2511101625_{str(self.num)}.npz', Pose_list=self.Pose_pre, Time=self.Time, v_pred=self.v_pred_pre, scene=self.scene,
                             xs=self.xs, ys=self.ys)
                ...
                if self.N == 2:
                    ##保存相关数据进行分析
                    np.savez(f'contact_slam_N2data_2511101625_{str(self.num)}.npz', Pose_list=self.Pose, Time=self.Time, v_pred=self.v_pred, scene=self.scene,
                             xs=self.xs, ys=self.ys)
                ...
                if self.count < self.window_size:  ##计算最近若干个值的均值
                    ...
                    if self.mu * f_ref * 0.9 < np.linalg.norm(
                        ...
                        if self.N == 0:
                            ##保存相关数据进行分析
                            np.savez(f'contact_slam_N0data_2511101625_{str(self.num)}.npz', Pose_list=self.Pose,
                                    Time=self.Time, v_pred=self.v_pred, scene=self.scene,
                                     xs=self.xs, ys=self.ys)
                            ...
                            ##保存相关数据进行分析
                            np.savez(f'contact_slam_N1data_2511101625_{str(self.num)}.npz', Pose_list=self.Pose, Time=self.Time, v_pred=self.v_pred, scene=self.scene,
                                     xs=self.xs, ys=self.ys)
                    ...
                    if self.n_now < self.X.shape[0]:  ##轨迹追踪还没有进行完
                        ...
                        if np.linalg.norm(f_projec) > self.mu * f_ref and (f_vec @ self.v_vec) < 0 and np.linalg.norm(
                            ...
                            if self.N == 0:  ##处于竖直向下运动阶段
                                ...
                                if f_z < 0.4 and self.terminate_flag == False:
