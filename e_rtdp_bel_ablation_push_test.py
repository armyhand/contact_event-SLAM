...
"""
使用 E-RTDP-Bel 经验启发式的 blind push block 消融实验
"""
...
class Motion_planning:
    ...
    def e_rtdp_bel_action(self, particles, weights, objects, history=(), max_depth=8):
        """执行经验策略 rollout，并用 RTDP-Bel Bellman 更新求解新 belief。"""
        ...
        def solve(ids, depth):
            ...
            total_probability = np.sum(probabilities[list(ids)])
            best_value, best_action = np.inf, None
            for action in self.actions:
                branches = {}
                for i in ids:
                    branches.setdefault(signature(i, action), []).append(i)
                if len(branches) == 1:
                    continue
                expected = 1.0
                for branch in branches.values():
                    branch_probability = (
                        np.sum(probabilities[branch]) / total_probability
                    )
                    expected += branch_probability * solve(tuple(branch), depth - 1)
        ...
        root_value = memo.get(
            (belief_key, max_depth), self._experience_heuristic(belief_key)
        )
        print(
            f"E-RTDP-Bel: hypotheses={len(belief_key)}, value={root_value:.3f}, action={action}"
        )
        return action
...
if __name__ == "__main__":
    ...
    for p in range(14, 16):
        # 每次迭代的新目录
        folder = f"heuristic_search_push/ablation_group2_{p}"  # 或用时间戳
        ...
        history = []
        # v_pred = M_planning.e_rtdp_bel_action(particles, weights, Movable_objects,
