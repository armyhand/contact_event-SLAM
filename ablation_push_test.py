...
def scene_plot(scene, xs, ys, object_mask, obstacles, object_pos, ref_point, particles, weights):
    ...
    # 添加这一行保存图片
    plt.savefig(os.path.join(folder, f'contact_slam_data_2511101625_{str(num)}_{str(j)}.png'), dpi=300, bbox_inches='tight')
    # plt.show()
...
if __name__ == "__main__":
    ...
    Movable_objects = [
            np.array([[-12.5, 12.5], [-12.5, -12.5], [12.5, -12.5], [12.5, 12.5]], dtype=float),  # 小矩形1
        ]
            ...
            while z == 0:
                ...
                if z_obs == 0:
                    ...
                    if action_list.shape[0] > 0 and np.linalg.norm(Pos_list[-1] - Pos_list[0]) > distance_thr:
                        ...
                        z = 1
                        ##保存相关数据进行分析
                        np.savez(os.path.join(folder, f'contact_slam_N2data_2511101625_{str(num)}_{str(j)}.npz'), particles=particles,
