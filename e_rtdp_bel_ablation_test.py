...
"""
使用 E-RTDP-Bel 经验启发式进行主动探索（基于原 ablation_test.py）
"""
...
class Motion_planning:
    def __init__(self, d, padding):
        ...
        self.r = 1.5  ##重采样半径范围
        ## E-RTDP-Bel 参数与跨场景经验库。belief 使用加权位姿单元表示，
        ## 策略按 history -> action 保存，以便不同初始 belief 复用。
        self.epsilon = 1.2
        self.belief_resolution = 1.0
        self.distance_resolution = max(1.0, 4 * self.d)
        self.experience_values = {}
        self.experience_policy = {}
        self.experience_beliefs = set()
    ...
    ## E-RTDP-Bel 启发式探索---------------------------------------
    def belief_from_particles(self, particles, weights):
        """将粒子压缩为有限的加权位姿假设，供 belief 规划使用。"""
        if len(particles) == 0:
            return tuple(), np.zeros((0, 2)), np.zeros(0)
        weights = np.asarray(weights, dtype=float)
        weights = weights / max(np.sum(weights), 1e-12)
        extent = np.ptp(particles, axis=0)
        adaptive_resolution = max(
            self.belief_resolution, math.sqrt(max(np.prod(extent), 1.0) / 96.0)
        )
        cells = np.rint(np.asarray(particles) / adaptive_resolution).astype(int)
        unique_cells, inverse = np.unique(cells, axis=0, return_inverse=True)
        cell_weights = np.bincount(
            inverse, weights=weights, minlength=len(unique_cells)
        )
        centers = np.zeros((len(unique_cells), 2), dtype=float)
        for i in range(len(unique_cells)):
            ids = inverse == i
            local_weights = weights[ids]
            centers[i] = np.average(particles[ids], axis=0, weights=local_weights)
        keep = cell_weights >= max(1e-6, 0.25 / len(unique_cells))
        centers, cell_weights, unique_cells = (
            centers[keep],
            cell_weights[keep],
            unique_cells[keep],
        )
        cell_weights /= max(np.sum(cell_weights), 1e-12)
        ## key 仍用物理坐标量化，避免自适应分辨率变化破坏经验匹配。
        key_cells = np.rint(centers / self.belief_resolution).astype(int)
        key = tuple(map(tuple, key_cells.tolist()))
        return key, centers, cell_weights

    def _contact_signature(self, hypothesis, action, objects):
        """预测一个位姿假设沿动作射线首次产生的离散观测。"""
        _, _, contact_pairs = self.select_pairs(action, objects)
        distances = []
        for pair in contact_pairs:
            low, high = np.asarray(pair[2][0], float), np.asarray(pair[2][1], float)
            low, high = np.minimum(low, high), np.maximum(low, high)
            low[np.isclose(low, high)] -= 2.0
            high[np.isclose(low, high)] += 2.0
            t_enter, t_exit = -np.inf, np.inf
            for axis in range(2):
                if abs(action[axis]) < 1e-12:
                    if not low[axis] <= hypothesis[axis] <= high[axis]:
                        t_enter, t_exit = 1.0, 0.0
                        break
                else:
                    t1 = (low[axis] - hypothesis[axis]) / action[axis]
                    t2 = (high[axis] - hypothesis[axis]) / action[axis]
                    t_enter, t_exit = max(t_enter, min(t1, t2)), min(
                        t_exit, max(t1, t2)
                    )
            if t_exit >= max(t_enter, 0.0):
                distances.append(max(t_enter, 0.0))
        if distances:
            return 1, int(round(min(distances) / self.distance_resolution))
        boundary = np.where(
            action > 0, [self.x_max, self.y_max], [self.x_min, self.y_min]
        )
        distance = float(np.dot(action, boundary - hypothesis))
        return -1, int(round(max(distance, 0.0) / self.distance_resolution))

    @staticmethod
    def _domain_heuristic(number_of_hypotheses):
        ## 一次二值接触观测最多将 belief 二分，因此这是 admissible 下界。
        return 0.0 if number_of_hypotheses <= 1 else math.log2(number_of_hypotheses)

    def _experience_heuristic(self, belief_key):
        """公式 3.4：扫描 B^E 与目标集合，计算 epsilon 膨胀启发值。"""
        value = self.epsilon * self._domain_heuristic(len(belief_key))
        for old_key in self.experience_beliefs:
            ## 以不确定性层级之差作为 belief 间的一致下界，避免把坐标差误作动作代价。
            jump = abs(
                self._domain_heuristic(len(belief_key))
                - self._domain_heuristic(len(old_key))
            )
            value = min(
                value, self.epsilon * jump + self.experience_values.get(old_key, 0.0)
            )
        return value

    def e_rtdp_bel_action(self, particles, weights, objects, history=(), max_depth=8):
        """用经验启发式 Bellman 更新选择动作，并把新策略写回经验库。"""
        belief_key, hypotheses, probabilities = self.belief_from_particles(
            particles, weights
        )
        if len(belief_key) <= 1:
            return np.array([0, 0])

        old_action = self.experience_policy.get(tuple(history))
        if old_action is not None:
            signatures = {
                self._contact_signature(h, np.asarray(old_action), objects)
                for h in hypotheses
            }
            if len(signatures) > 1:
                action = np.asarray(old_action, dtype=int)
                print(
                    f"E-RTDP-Bel rollout: hypotheses={len(belief_key)}, action={action}"
                )
                return action
        ...
        def signature(i, action):
            cache_key = (i, tuple(action.tolist()))
            if cache_key not in signature_cache:
                signature_cache[cache_key] = self._contact_signature(
                    hypotheses[i], action, objects
                )
            return signature_cache[cache_key]

        def solve(ids, depth):
            state_key = tuple(belief_key[i] for i in ids)
            if len(ids) <= 1:
                return 0.0
            if depth == 0:
                return self._experience_heuristic(state_key)
            cache_key = (state_key, depth)
            if cache_key in memo:
                return memo[cache_key]
            local_prob = probabilities[list(ids)]
            local_prob /= np.sum(local_prob)
            best_value, best_action = np.inf, None
            for action in self.actions:
                branches = {}
                for i in ids:
                    obs = signature(i, action)
                    branches.setdefault(obs, []).append(i)
                if len(branches) == 1:
                    continue
                expected = 1.0
                for branch in branches.values():
                    branch_prob = np.sum(probabilities[branch]) / np.sum(
                        probabilities[list(ids)]
                    )
                    expected += branch_prob * solve(tuple(branch), depth - 1)
                if expected < best_value:
                    best_value, best_action = expected, action
            if best_action is None:
                best_value = self._experience_heuristic(state_key)
            memo[cache_key] = best_value
            policy[state_key] = (
                None if best_action is None else tuple(best_action.tolist())
            )
            return best_value

        root_ids = tuple(range(len(belief_key)))
        solve(root_ids, max_depth)
        action_tuple = self.experience_policy.get(
            tuple(history), policy.get(belief_key)
        )
        if action_tuple is None or not any(
            np.array_equal(action_tuple, a) for a in self.actions
        ):
            action_tuple = policy.get(belief_key)
        if action_tuple is None:
            ## 对称场景中若所有动作观测均相同，使用确定性安全回退。
            action_tuple = tuple(self.actions[0].tolist())
        for state_key, action in policy.items():
            self.experience_beliefs.add(state_key)
            computed = [value for (key, _), value in memo.items() if key == state_key]
            self.experience_values[state_key] = (
                min(computed) if computed else self._experience_heuristic(state_key)
            )
        self.experience_policy[tuple(history)] = action_tuple
        action = np.asarray(action_tuple, dtype=int)
        print(
            f"E-RTDP-Bel: hypotheses={len(belief_key)}, value={memo.get((belief_key, max_depth), 0):.3f}, action={action}"
        )
        return action
...
if __name__ == "__main__":
    ...
    for p in range(4, 10):
        ...
        history = []
        # v_pred = M_planning.e_rtdp_bel_action(
        #     particles, weights, Movable_objects, history=tuple(history)
        # )
        v_pred = np.array([0, 1])
        while (
            ...
            # 根据执行历史 rollout 旧策略，并以 E-RTDP-Bel 经验启发式选择下一动作
            peaks = M_planning.find_local_maximum(particles, weights, radius=5)
            print("len(peaks)=", len(peaks))
            history.append((tuple(v_pred.tolist()), int(Z_obs[-1])))
            v_pred = M_planning.e_rtdp_bel_action(
                particles_now, weights, Movable_objects, history=tuple(history)
            )
