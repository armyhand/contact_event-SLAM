...
if __name__ == "__main__":
    ...
    # ]
    Movable_objects = [np.array([[-0.75, 5.98-12.38], [0.75, 5.98-12.38], [0.75, 0.0], [-0.75, 0.0]])]
    R = np.array([[0, -1], [1, 0]])  # 顺时针90°旋转矩阵
    rotated = []
    ...
    while terminate_flag == 0: ##如果峰值数量不是1,那么进入主动探索阶段
        ##绘图
        ...
        while z == 0:
            ...
            if z_obs == -1:
                ...
                z = 1
                ##保存相关数据进行分析
                np.savez(f'contact_slam_N2data_2511101625_{str(num)}.npz', particles=particles,
            ...
            if z_obs == 1:
                ...
                z = 1
                ##保存相关数据进行分析
                np.savez(f'contact_slam_N1data_2511101625_{str(num)}.npz', particles=particles,
                         weights=weights, Pose_list=Pos_list, particles_filter=particles_filter,
                         Z_list=Z_obs, v_pred=v_pred, scene=scene, xs=xs, ys=ys)
